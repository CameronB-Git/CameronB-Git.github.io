...
document.addEventListener("DOMContentLoaded", function() {
    ...
    fetch('/Assets/sites/reckittSite/src/products.json')
...
function addItemToBasket() {
    if (selectedProductIndex === undefined) {
        ...
        productsInBasket = JSON.parse(localStorage.getItem("item")) || [];
    
        productsInBasket.push(selectedProductIndex);
        localStorage.setItem("item", JSON.stringify(productsInBasket));
...
function displayProductsInBasket() {
    ...
    if (Array.isArray(selectedProductIndex) && selectedProductIndex.length > 0) {
        for (let i = 0; i < selectedProductIndex.length; i++) {
            ...
            // If Product Name Contains "Capsules" --> Make Image Column Larger 
